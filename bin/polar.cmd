@echo off
chcp 65001 >nul 2>&1
REM Polar - Local AI Coding Agent
...
set TEMP_DIR=%TEMP%\polar-upgrade-%RANDOM%
...
if %errorlevel%==0 (
    ...
    if exist "%TEMP_DIR%\dlc.jar" (
        curl -fsSL -o "%TEMP_DIR%\polar.cmd" "%RAW_BASE_URL%/bin/polar.cmd" 2>nul
...
if exist "%TEMP_DIR%\dlc.jar" (
    powershell -Command "try { Invoke-WebRequest -Uri '%RAW_BASE_URL%/bin/polar.cmd' -OutFile '%TEMP_DIR%\polar.cmd' -UseBasicParsing } catch {}" 2>nul
...
if exist "%TEMP_DIR%\polar.cmd" copy /Y "%TEMP_DIR%\polar.cmd" "%DLC_HOME%\bin\polar.cmd" >nul
